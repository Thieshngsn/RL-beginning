...
paddle.goto(0, -265)        # Place the shape on bottom of the screen
...
ball.penup()
ball.goto(0, -156)           # Place the shape in middle

# Paddle Movement
def paddle_right():
    x = paddle.xcor()        # Get the x position of paddle
    if x < 280:
        paddle.setx(x+5)    # increment the x position by 20

def paddle_left():
    x = paddle.xcor()        # Get the x position of paddle
    if x > -290:
        paddle.setx(x-5)    # decrement the x position by 20
...
while True:   # same loop4
    ...
    if abs(ball.ycor() + 250) < 2 and abs(paddle.xcor() - ball.xcor()) < 55:
        ...
        if random.random() > 0.5:
            ball.dx = random.random() * 8 + 2  # ball's x-axis velocity
        else:
            ball.dx = -random.random() * 8 -2   # ball's x-axis velocity

    # Ball-Ground collison            
    if ball.ycor() < -290:   # If ball touch the ground 
        ball.goto(0, -165)    # Reset the ball position  
        ball.dx = random.random() * 4 + 3   # ball's x-axis velocity 
        ball.dy = 5  # ball's y-axis velocity 
